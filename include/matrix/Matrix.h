...
class Matrix
{
  public:
    ...
    Matrix &operator=(const BJtensor &T);
...
{ 
  ...
  if ((row < 0) || (row >= numRows)) {
    std::cerr << "Matrix::operator() - row " << row << " out of range [0, " <<  numRows-1 << endln;
    return data[0];
  } else if ((col < 0) || (col >= numCols)) {
    std::cerr << "Matrix::operator() - row " << col << " out of range [0, " <<  numCols-1 << endln;
