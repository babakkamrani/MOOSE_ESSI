    ...
    :numRows(0), numCols(0), dataSize(0), data(0), fromFree(0)
{
    ...
    if (dataSize != 0)
    {
        ...
        if (data == 0)
        {
            ...
            std::cerr << "Ran out of memory on init of size " << dataSize ;
            numRows = 0;
            numCols =0;
            dataSize = 0;
        }
...
Matrix::Solve(const Vector &b, Vector &x) const
{
    ...
    if (n != x.Size())
    {
        std::cerr << "Matrix::Solve(b,x) - dimension of x, " << numRows << "is not same as matrix " <<  x.Size() ;
        return -2;
    }

    if (n != b.Size())
    {
        std::cerr << "Matrix::Solve(b,x) - dimension of x, " << numRows << "is not same as matrix " <<  b.Size() ;
        return -2;
    }
#endif
...
Matrix::Solve(const Matrix &b, Matrix &x) const
{
    ...
    if (n != x.numRows)
    {
        std::cerr << "Matrix::Solve(B,X) - #rows of X, " << x.numRows << " is not same as the matrices: " << numRows ;
        return -2;
    }

    if (n != b.numRows)
    {
        std::cerr << "Matrix::Solve(B,X) - #rows of B, " << b.numRows << " is not same as the matrices: " << numRows ;
        return -2;
    }

    if (x.numCols != b.numCols)
    {
        std::cerr << "Matrix::Solve(B,X) - #cols of B, " << b.numCols << " , is not same as that of X, b " <<  x.numCols ;
        return -3;
    }
#endif
...
Matrix::Invert(Matrix &theInverse) const
{
    ...
    {
        std::cerr << "Matrix::Solve(B,X) - #rows of X, " << numRows<< ", is not same as matrix " << theInverse.numRows ;
        return -2;
    }
#endif
...
Matrix &
Matrix::operator=(const BJtensor &V)
{
    int rank = V.rank();
    if (rank != 4)
    {
...
Matrix::operator/=(double fact)
{
    ...
    else
    {
        ...
        std::cerr << MATRIX_VERY_LARGE_VALUE ;
