...
//#endif    
  ...
  if (data == 0) 
  {
    cerr << "ID::ID(int): ran out of memory with size " << size ;
    exit(-1);
  }
...
//#ifdef _G3DEBUG
  ...
  if (arraySz < sz) {
    cerr << "ID::ID(size, arraySize) - arraySize " << arraySz  << " specified < " << size ;
    arraySz = sz;
  }
//#endif    
  ...
  if (data == 0) {
    cerr << "ID::ID(int, int): ran out of memory with arraySize: " << arraySize ;
    exit(-1);
  }
  ...
  :sz(other.sz), data(0), arraySize(other.arraySize), fromFree(0)
{
  ...
  data = new int[arraySize]; 
  if (data == 0) {
    cerr << "ID::ID(ID): ran out of memory with arraySize " << arraySize ,
    exit(-1);
  }
...
//#endif
  ...
  if (x >= arraySize) {
	...
	newData[j] = 0;
      ...
      cerr << "ID::[]): ran out of memory with arraySize " << arraySize ;
      return ID_NOT_VALID_ENTRY;
    }
  }
...
ID::operator=(const ID &V) 
{
	...
	if (sz != V.sz) {
		...
		if (data == 0) {
		    ...
		    cerr << "for new array of size" << arraySize ;
		    sz = 0;
		    arraySize = 0;
		}
	    }
