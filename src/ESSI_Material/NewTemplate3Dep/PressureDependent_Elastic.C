...
const BJtensor& PressureDependent_Elastic::getElasticStiffness (const MaterialParameter& MaterialParameter_in) const
{
    ...
    if (v >= 0.5 || v < -1.0)
    {
        cerr << "Warning!! PressureDependent_Elastic: Invalid possoin's ratio. \n";
        exit (1);
    }
    ...
    if ( p_ref <= 0.0)
    {
        cerr << "Warning!! PressureDependent_Elastic: Invalid reference pressure. \n";
        exit (1);
    }
...
double PressureDependent_Elastic::getE0(const MaterialParameter& MaterialParameter_in) const
{
    ...
    {
        cerr << "PressureDependent_Elastic: Invalid Input. \n";
        exit (1);
    }
    else
    {
